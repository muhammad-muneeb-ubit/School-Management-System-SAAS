...
export default function DashboardLayout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex">
      ...
      <div className={` ${sidebarOpen ? 'w-64' : 'w-15'} fixed left-0 top-0 h-screen bg-blue-800 text-white transition-all duration-300 flex flex-col z-50`}>
        {/* Sidebar Header */}
        <div className="p-4 flex items-center justify-between border-b border-blue-700 shrink-0">
          {sidebarOpen && (
        ...
        <nav className="p-2 space-y-1 overflow-y-auto">
          {menuItems.map((item) => (
            <Link key={item.name} to={item.path} className={`flex items-center space-x-2 p-2 rounded hover:bg-blue-700 transition ${location.pathname === item.path ? 'bg-blue-900' : ''}`}>
              <item.icon size={20} />
              {sidebarOpen && (
                <span>{item.name}</span>
              )}
