...
export default function CollapseSidebarSkeleton({
  sidebarOpen = true,
  user,
}) {
  let menuCount = 7;

  if (user === 'Principal' || user?.role === 'Principal') {
    menuCount = 13;
  } else if (user === 'Teacher' || user?.role === 'Teacher') {
    menuCount = 6;
  } else if (user === 'Parent' || user?.role === 'Parent') {
    menuCount = 6;
  } else if (user === 'Super Admin' || user?.role === 'Super Admin') {
    menuCount = 4;
  }

  const items = Array.from({ length: menuCount });
